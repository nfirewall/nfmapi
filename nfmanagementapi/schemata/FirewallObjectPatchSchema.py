...
from app import ma
from nfmanagementapi.models import FirewallObject

class FirewallObjectPatchSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        model = FirewallObject
        ordered = True
    
    uuid = fields.UUID(required=True, description="Unique Identifier", dump_only=True)
    name = fields.String(required=False, description="Firewall name")
    description = fields.String(required=False, description="Description")
    primary_address = fields.String(required=False, description="Primary IP address")
